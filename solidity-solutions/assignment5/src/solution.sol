...
library Pairing {
    ...
    function create_G2Point(uint256 scalar) public view returns (G2Point memory) {
        return scalar_mul_G2(getG2(), scalar);
    }
    ...
    */
    ...
    function scalar_mul_G2(G2Point memory p, uint256 s) internal view returns (G2Point memory r) {
        ...
        bool success;
        assembly {
            success := staticcall(sub(gas(), 2000), 18, input, 0xA0, r, 0x80)
            switch success case 0 { invalid() }
        }
        require(success, "pairing-mul-g2-failed");
    }
...
contract Verifier {
    ...
    function verifyingKey() internal view returns (VerifyingKey memory vk) {
        ...
        Pairing.G2Point memory beta2 = Pairing.create_G2Point(scalar_beta2);
        Pairing.G2Point memory gamma2 = Pairing.create_G2Point(scalar_gamma2);
        Pairing.G2Point memory delta2 = Pairing.create_G2Point(scalar_delta2);
